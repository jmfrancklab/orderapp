...
_BOOKMARKLET = (
    ...
    r"var mcm=/(^|\.)mcmaster\.com$/i.test(window.location.hostname);"
    r"var mcmPart=null,mcmScope=null;"
    r"if(mcm){"
    # McMaster leaves the address bar on the family page when its inline order
    # box opens.  The box's Product Detail link is the selected part's URL.
    r"mcmPart=document.querySelector('a[class*=\"selectedPartNumberLinkOrderBox\"][href],"
    r"a[class*=\"productDetailLink\"][href]');"
    r"if(mcmPart)url=mcmPart.href;"
    r"}else{"
    ...
    r"if(mcm){"
    # Scope extraction to the open inline order box when present.  CSS-module
    # hashes change, but McMaster's descriptive class-name prefixes are stable.
    r"var row=mcmPart&&mcmPart.closest('tr');"
    r"var inDetail=mcmPart&&/productDetailLink/.test(mcmPart.className);"
    r"mcmScope=inDetail?row:(row&&row.nextElementSibling?row.nextElementSibling:document);"
    r"var de=mcmScope.querySelector('[class*=\"productDescription\"]');"
    r"if(de)desc=de.textContent.trim();"
    r"else{var h1=document.querySelector('[class*=\"productDetailHeaderPrimary\"]');"
    r"var h3=document.querySelector('[class*=\"productDetailHeaderSecondary\"]');"
    r"if(h1)desc=(h1.textContent+' '+(h3?h3.textContent:'')).trim();}"
    r"if(!desc&&row){var pc=row.parentElement;"
    r"while(pc&&pc!==document.body&&!pc.querySelector('h2[title]'))pc=pc.parentElement;"
    r"var ph=pc&&pc.querySelector('h2[title]');"
    r"if(ph)desc=(ph.textContent+' '+mcmPart.textContent).trim();}"
    r"var pe=(row&&row.querySelector('[class*=\"priceCell\"]'))||"
    r"mcmScope.querySelector('[class*=\"_price_\"]');"
    r"if(pe){var mm=(pe.textContent||'').match(/([\d,]+\.\d{2})/);"
    r"if(mm)price=mm[1].replace(/,/g,'');}}"
